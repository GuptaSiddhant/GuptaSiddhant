...
export function generateKeyDownArray({
  ...
  if (shiftKey) keys.push(ShortcutModifier.SHIFT)
  if (metaKey) keys.push(ShortcutModifier.META)
  if (altKey) keys.push(ShortcutModifier.ALT)
  if (ctrlKey) keys.push(ShortcutModifier.CONTROL)
...
export function matchKeyDownArrays(
  ...
  for (let i = 0; i < normalisedA.length; i++) {
    if (normalisedA[i] !== normalisedB[i]) return false
  }
